...
func (r *PostgreSQLRepository) RegisterInway(model *domain.Inway) error {
	type registerParams struct {
		...
		ManagementAPIProxyAddress string    `db:"inway_management_api_proxy_address"`
...
func prepareRegisterInwayStmt(db *sqlx.DB) (*sqlx.NamedStmt, error) {
	query := `
		...
		insert into directory.inways
		    		(name, organization_id, address, management_api_proxy_address, version, created_at, updated_at)
			 select :inway_name, organization.id, :inway_address,:inway_management_api_proxy_address, nullif(:inway_version, ''), :inway_created_at, :inway_updated_at
			   from organization
