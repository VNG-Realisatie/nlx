...
// WaitUntilLatestDirectoryDBVersion blocks until the database is migrated to the latest version.
// This function can exit the program when failing to query postgres for the version.
func WaitUntilLatestDirectoryDBVersion(logger *zap.Logger, db *sql.DB) {
	...
	for {
		...
		if version != LatestDirectoryDBVersion {
			logger.Info(fmt.Sprintf("db is at version %d, require version %d", version, LatestDirectoryDBVersion))
			time.Sleep(1 * time.Second)
			continue
		}
		break
