...
First, let's clone the NLX project inside the `nlx-setup` directory. It contains the Docker Compose file and its dependencies.
...
Ensure the file permissions are locked down. The NLX components will print warning messages when the file permissions are too broad.
The containers run as UID: 1001, which means this must be owner of the private keys.

```bash
sudo chmod 600 ../org.key
sudo chown 1001 ../org.key

sudo chmod 600 ./pki/certs/internal-cert-key.pem
sudo chown 1001 ./pki/certs/internal-cert-key.pem
```
