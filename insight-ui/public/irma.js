...
function connectClientToken() {
    ...
    if (ua === UserAgent.Android) {
        var intent = "intent://" + url + "#Intent;package=org.irmacard.cardemu;scheme=cardemu;" + "l.timestamp=" + Date.now() + ";" + "S.qr=" + encodeURIComponent(JSON.stringify(sessionPackage)) + ";" + "S.browser_fallback_url=http%3A%2F%2Fapp.irmacard.org%2Fverify;end";
...
function init(irmaapiserver) {
    ...
    var iloglevel = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : Loglevel.Info;
