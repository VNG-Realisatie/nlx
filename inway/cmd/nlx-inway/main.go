...
import (
	...
	"github.com/VNG-Realisatie/nlx/logdb/logdbversion"
)
...
func main() {
	...
	logdbversion.WaitUntilLatestVersion(logger, db.DB)

	iw, err := inway.NewInway(logger, db, options.SelfAddress, options.TLSOptions, options.DirectoryAddress)
	if err != nil {
