...
func TestRegisterOutway(t *testing.T) {
	...
	var tests = map[string]struct {
		...
		"when_providing_an_empty_self_address_api": {
			args: args{
				request: &api.RegisterOutwayRequest{
					Name:           "outway42.basic",
					PublicKeyPEM:   testPublicKeyPEM,
		...
		"when_providing_an_empty_listen_address": {
			args: args{
				request: &api.RegisterOutwayRequest{
					Name:           "outway42.basic",
					PublicKeyPEM:   testPublicKeyPEM,
			...
			wantErr: status.Error(codes.InvalidArgument, "invalid outway: listenAddress: cannot be blank."),
		},
		"happy_flow_ipv4": {
			args: args{
				database: &database.Outway{
