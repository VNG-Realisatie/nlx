...
func (s *RoundRobinLoadBalancedHTTPService) LogServiceErrors(w http.ResponseWriter, r *http.Request, e error) {
	s.logger.Error(fmt.Sprintf("failed request to '%s', try again later and check your firewall, check O1 and M1 at https://docs.nlx.io/support/common-errors/", r.URL.String()), zap.Error(e))
