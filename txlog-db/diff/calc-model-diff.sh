...
tmpdir=`mktemp -d -t txlog_db_diff-XXXXXXXX`
...
function cleanupDockerContainers {
  docker kill txlog_diff_migrate &> /dev/null || true
  docker rm txlog_diff_migrate &> /dev/null || true
  docker kill txlog_diff_target &> /dev/null || true
  docker rm txlog_diff_target &> /dev/null || true
}
cleanupDockerContainers
pgTargetContainer=`docker run --name txlog_diff_target ${dockerNetwork} -e POSTGRES_PASSWORD=postgres -d postgres:9.6`
pgMigrateContainer=`docker run --name txlog_diff_migrate ${dockerNetwork} -e POSTGRES_PASSWORD=postgres -d postgres:9.6`
...
until (psql --user postgres --host ${pgTargetIP} --command '\l'&>/dev/null); do
  echo "Waiting for postgres in container txlog_diff_target"
  sleep 1
done
psql --echo-errors --variable "ON_ERROR_STOP=1" --user postgres --host ${pgTargetIP} --command "CREATE DATABASE txlog;" postgres >/dev/null
psql --echo-errors --variable "ON_ERROR_STOP=1" --user postgres --host ${pgTargetIP} --file "model/txlog.sql" txlog >/dev/null
pg_dumpall --user postgres --host ${pgTargetIP} --roles-only > ${tmpdir}/target.sql
pg_dump --user postgres --host ${pgTargetIP} --schema-only txlog >> ${tmpdir}/target.sql
...
until (psql --user postgres --host ${pgMigrateIP} --command '\l' &>/dev/null); do
  echo "Waiting for postgres in container txlog_diff_migrate"
  sleep 1
done
psql --echo-errors --variable "ON_ERROR_STOP=1" --user postgres --host ${pgMigrateIP} --command "CREATE DATABASE txlog;" postgres >/dev/null
migrateDSN="postgres://postgres:postgres@${pgMigrateIP}:5432/txlog?sslmode=disable"
...
pg_dump --user postgres --host ${pgMigrateIP} --schema-only --exclude-table schema_migrations txlog >> ${tmpdir}/migrate.sql
...
echo "Verifying db/dbversion const LatestTxlogDBVersion"
if ! grep -q "^const LatestTxlogDBVersion = ${migrateLastVersion}\$" dbversion/version.go; then
    echo "dbversion/version.go is invalid, expected LatestTxlogDBVersion to equal ${migrateLastVersion}, got different value.";
    exit 64
fi
